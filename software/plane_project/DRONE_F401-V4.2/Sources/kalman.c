...
extern void kalman2_init(kalman2_state *state,float dt)
{
    ...
    state->q[0]    = 10e-3;
    state->q[1]    = 10e-4;
    state->r       = 1;  /* estimated error convariance */
}
...
extern float kalman2_filter(kalman2_state *state, float z_measure,float acc_measure,float dt)
{
    ...
    /* Step1: Predict */
    state->x[0] = state->A[0][0] * state->x[0] + state->A[0][1] * state->x[1]+0.5*acc_measure*dt*dt;
    state->x[1] = state->A[1][0] * state->x[0] + state->A[1][1] * state->x[1]+acc_measure*dt;
    /* p(n|n-1)=A*p(n-1|n-1)*A^T+q */
    state->p[0][0] = (state->A[0][0] * state->p[0][0] + state->A[0][1] * state->p[1][0] )*state->A[0][0] +(state->A[0][0] * state->p[0][1] + state->A[0][1] * state->p[1][1] )*state->A[0][1] + state->q[0];
    state->p[0][1] = (state->A[0][0] * state->p[0][0] + state->A[0][1] * state->p[1][0] )*state->A[1][0] +(state->A[0][0] * state->p[0][1] + state->A[0][1] * state->p[1][1] )*state->A[1][1];
    state->p[1][0] = (state->A[1][0] * state->p[0][0] + state->A[1][1] * state->p[1][0] )*state->A[0][0] +(state->A[1][0] * state->p[0][1] + state->A[1][1] * state->p[1][1] )*state->A[0][1];
    state->p[1][1] = (state->A[1][0] * state->p[0][0] + state->A[1][1] * state->p[1][0] )*state->A[1][0] +(state->A[1][0] * state->p[0][1] + state->A[1][1] * state->p[1][1] )*state->A[1][1] + state->q[1];

    /* Step2: Measurement */
    /* gain = p * H^T * [r + H * p * H^T]^(-1), H^T means transpose. */
    temp0 = state->p[0][0] * state->H[0] + state->p[1][0] * state->H[1];
    temp1 = state->p[0][1] * state->H[0] + state->p[1][1] * state->H[1];
    temp  = state->r + state->H[0] * temp0 + state->H[1] * temp1;
    state->gain[0] = (state->p[0][0] * state->H[0] + state->p[0][1] * state->H[1])/ temp;
    state->gain[1] = (state->p[1][0] * state->H[0] + state->p[1][1] * state->H[1]) / temp;
    ...
    state->p[0][1] = (0 - state->gain[0] * state->H[1]) * state->p[0][1];
    state->p[1][0] = (0 - state->gain[1] * state->H[0]) * state->p[1][0];
