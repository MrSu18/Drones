...
void spl06_pressure_rate_config(uint8_t background_rate,uint8_t oversamply)
{
    ...
    {
        uint8_t data;
        data = spl0601_read(SPL06_ADDR, CFG_REG);//读取原寄存器值
        data |= 0X04;//P-SHIFT位置1
        spl0601_write(SPL06_ADDR, CFG_REG, data);//重新写回寄存器          
    }    
    switch(oversamply)
    {
    ...
    spl0601_write(SPL06_ADDR, PRS_CFG_REG, data);//写入配置
}
...
void spl06_temperature_rate_config(uint8_t background_rate,uint8_t oversamply,uint8_t ext)
{
    ...
    spl0601_write(SPL06_ADDR, TMP_CFG_REG, data);//写入配置
}
/***********************************************************************
 * 传感器测量模式配置与状态读取
 * @param[in] 
